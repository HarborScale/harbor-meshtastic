...
def normalize_metrics(node_id, node):
    ...
    # 2. Battery & Voltage (Device Metrics)
    if 'deviceMetrics' in node:
        dm = node['deviceMetrics']
        if 'batteryLevel' in dm:
            data['battery_level'] = dm['batteryLevel']
        if 'voltage' in dm:
            data['voltage'] = dm['voltage']
        if 'channelUtilization' in dm:
            data['channel_util'] = dm['channelUtilization']
        if 'airUtilTx' in dm:
            data['air_util_tx'] = dm['airUtilTx']

    # 3. Environment (Temp/Hum/Pressure)
    if 'environmentMetrics' in node:
        em = node['environmentMetrics']
        if 'temperature' in em:
            data['temperature'] = em['temperature']
        if 'relativeHumidity' in em:
            data['humidity'] = em['relativeHumidity']
        if 'barometricPressure' in em:
            data['pressure'] = em['barometricPressure']

    # 4. Position (Lat/Lon/Alt)
    if 'position' in node:
        pos = node['position']
    ...
    if 'airQualityMetrics' in node:
        ...
        if 'pm25' in aq: data['aq_pm25'] = aq['pm25']
